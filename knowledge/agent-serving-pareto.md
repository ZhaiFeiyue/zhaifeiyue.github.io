...
```
TTFT(L) = 2 × active_params × L / total_FLOPS
```
...
```
step_time = max(T_weight_load + T_kv_load, T_compute)

T_weight_load = model_bytes / total_bandwidth          [恒定]
T_kv_load     = B × L_avg × kv_bytes_per_token / BW   [随 batch 和 context 增长]
T_compute     = 2 × active_params × B / total_FLOPS    [随 batch 增长]

TPOT = step_time  (每个 step 为每个 request 产生 1 个 token)
```
...
|------|-----------|---------------|----------------|
| TTFT (有 cache) | ~36ms | ~130ms | ~115ms |
| TTFT (无 cache) | ~380ms | ~2,500ms | ~4,000ms |
| TPOT | ~5.5ms | ~5.8ms | ~7.2ms |
| Per-turn latency R | ~740ms | ~500ms | ~29.6s |
| GPU 利用率 | ~67% | ~12% | ~82% |
| Max sessions (KV) | ~73 | ~21 | ~13 |
| Token throughput/GPU | ~310 tok/s | ~12 tok/s | ~90 tok/s |
