...
- **KV-cache**: MLA 将 KV 压缩到 ~90 KB/token（vs GQA 的 320 KB），C_max 大幅增加
...
（注：K2.5 实际 61 层，来自 [HuggingFace config.json](https://huggingface.co/moonshotai/Kimi-K2.5/blob/main/config.json)）
...
- K2.5: [moonshotai/Kimi-K2.5](https://huggingface.co/moonshotai/Kimi-K2.5/blob/main/config.json)
- Llama: [meta-llama/Llama-3.1-*](https://huggingface.co/meta-llama)
