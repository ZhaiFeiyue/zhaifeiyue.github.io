...
@media (max-width: 768px) {
  ...
  body > nav .logo { font-size: 0.98rem; flex-shrink: 0; }
  body > nav a { font-size: 0.82rem; flex-shrink: 0; white-space: nowrap; }
  .w { padding: 0 14px 40px; }
}

/* Phone portrait: stack everything */
@media (max-width: 480px) {
  body > nav { gap: 14px; padding: 0 12px; height: 44px; }
  body > nav .logo { font-size: 0.92rem; }
  body > nav a { font-size: 0.78rem; }
...
def render_toc(toc_entries):
    """Render the inline Table of Contents placed right after TL;DR.
    Returns an HTML `<div>` block (2-column grid, collapsible details)
    or '' if there are too few entries to bother."""
    # Skip the TOC when there's nothing meaningful to navigate to
    # (very short / one-section notes).
    if len(toc_entries) < 3:
        return ''
    parts = ['<div class="toc-inline" aria-label="In-page table of contents">']
...
/* ---- In-page Table of Contents (inline block, after TL;DR) ---- */
.toc-inline {{
  max-width: 960px; margin: 0 auto 24px; padding: 0 24px;
}}
.toc-inline details {{
  background: #f8fafc; border: 1px solid var(--bd); border-radius: 10px;
  padding: 10px 18px; font-family: 'IBM Plex Sans', 'Noto Sans SC', sans-serif;
}}
.toc-inline summary {{
  cursor: pointer; font-weight: 700; font-size: 0.8rem;
  color: var(--ink, #1b2a4a); text-transform: uppercase; letter-spacing: 0.08em;
  padding: 2px 0; list-style: none; user-select: none;
}}
.toc-inline summary::-webkit-details-marker {{ display: none; }}
.toc-inline summary::before {{
  content: '▾'; margin-right: 8px; font-size: 0.7rem;
  display: inline-block; transition: transform .15s; color: var(--mt, #6b7280);
}}
.toc-inline details:not([open]) summary::before {{ transform: rotate(-90deg); }}
.toc-list {{
  display: grid; grid-template-columns: repeat(2, minmax(0, 1fr));
  gap: 0 20px; margin-top: 10px;
}}
.toc-list a {{
  display: block; padding: 5px 10px; border-radius: 4px;
  color: var(--mt, #6b7280); text-decoration: none;
  border-left: 2px solid transparent; font-size: 0.86rem;
  line-height: 1.4;
  transition: color .1s, background .1s, border-color .1s;
}}
.toc-list a:hover {{ color: var(--ink, #1b2a4a); background: #eef2ff; }}
.toc-list a.active {{
  color: var(--accent, #2563eb); font-weight: 600;
  border-left-color: var(--accent, #2563eb); background: #eff6ff;
}}
.toc-list .toc-lvl-2 {{ font-weight: 500; }}
@media (max-width: 768px) {{
  .toc-inline {{ padding: 0 14px; }}
